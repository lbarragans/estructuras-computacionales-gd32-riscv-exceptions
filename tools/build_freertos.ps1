...
foreach ($RequiredPath in @($SdkBuildScript, (Join-Path $SdkAppFolder "main.c"), (Join-Path $SdkAppFolder "app_cfg.h"), (Join-Path $SourceAppFolder "main.c"), (Join-Path $SourceAppFolder "app_cfg.h"), (Join-Path $NUCLEI_TOOLCHAIN_DIR "riscv-nuclei-elf-gcc.exe"), (Join-Path $OpenOcdBin "openocd.exe"))) {
...
foreach ($FileName in @("main.c", "app_cfg.h")) {
    ...
    if (-not (Test-Path $BackupPath)) { Copy-Item (Join-Path $SdkAppFolder $FileName) $BackupPath }
    $DestinationFile = Join-Path $SdkAppFolder $FileName
    Copy-Item (Join-Path $SourceAppFolder $FileName) $DestinationFile -Force
    # Evita que Make reutilice un objeto anterior cuando un ZIP conserva una
    # marca de tiempo futura o idéntica a la del objeto compilado.
